...
import CartSheet from "../[productId]/components/cart-sheets";
...
const RestaurantCategories = ({ restaurant }: RestaurantCategoriesProps) => {
  ...
  return (
    <div className="relative z-50 mt-[-1.5rem] rounded-t-3xl border bg-white">
      ...
      {products.length > 0 && (
        <div className="fixed botom-0 left-0 right-0 flex w-full items-center justify-between border-t bg-white px-5 py-3">
          <div>
            <p className="text-xs text-muted-foreground">Total dos pedidos</p>
            <p className="text-sm font-semibold">{formatCurrency(total)}<span className="text-xs font-normal text-muted-foreground">
              /{totalQuantity} {totalQuantity > 1 ? "itens" : "item"} </span></p>
          </div>
